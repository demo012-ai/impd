...
def predict_price():
    ...
    try:
        original_value = data_subset.loc[(data_subset['LSTAT'] == data['LSTAT']) &(data_subset['RM'] == data['RM']) &(data_subset['PTRATIO'] == data['PTRATIO']) &(data_subset['INDUS'] == data['INDUS']) &(data_subset['TAX'] == data['TAX']),'MEDV'].values[0]
    except Exception as e:
        original_value="NOT AVAILABLE"
    # print(row_numbers)
    # Load the trained model random forest
    # random_regressor = joblib.load(model_filename1)
    # # load the trained model linear regression
    # linear_regressor= joblib.load(model_filename)
    # Making predictions
    random_forest_prediction = rf.predict([features])
    linear_regressor_prediction = regressor.predict([features])
    return jsonify({'rfp': random_forest_prediction.tolist(),'lrp':linear_regressor_prediction.tolist(),'org':original_value})
